...
*                       the model
* Developed by:         Electricity Authorifty, New Zealand
...
Aliases to be aware of:
  ...
  i_energyOfferComponent = NRGofrCmpnt      i_ILRbidComponent = ILbidCmpnt
...
* Flag to apply corresponding vSPD model
  VSPDModel(tp)                                       '0=VSPD, 1=vSPD_BranchFlowMIP, 2=VSPD (last solve)'
...
* Dispatch results for reporting - Trade period level - Island output
  ...
  o_offerReserve_TP(dt,o)                             'Output MW reserve cleared for each trade period'
  o_offerFIR_TP(dt,o)                                 'Output MW effective FIR for each reserve offer for each trade period'
  o_offerSIR_TP(dt,o)                                 'Output MW effective SIR for each reserve offer for each trade period'
  o_offerLIR_TP(dt,o)                                 'Output MW effective LIR for each reserve offer for each trade period'
  ...
  o_offerReserveBlock_TP(dt,o,trdBlk,resT)            'Output MW reserve block cleared for each trade period'
  ...
  o_ResCleared_TP(dt,ild)                             'Reserve cleared from an island for each trade period'
  ...
  o_LIRreqd_TP(dt,ild)                                'Output MW required LIR for each trade period'
  o_FIRcleared_TP(dt,ild)                             'Output - total FIR cleared by island'
  o_SIRcleared_TP(dt,ild)                             'Output - total SIR cleared by island'
  o_LIRcleared_TP(dt,ild)                             'Output - total LIR cleared by island'
  o_ResPrice_TP(dt,ild)                               'Output $/MW price for each reserve classes for each trade period'
  ...
  o_LIRprice_TP(dt,ild)                               'Output $/MW price for SIR reserve classes for each trade period'
  ...
  o_LIRviolation_TP(dt,ild)                           'Violation MW for SIR reserve classes for each trade period'
...
* Virtual reserve output
  ...
  o_LIRvrMW_TP(dt,ild)                                'MW scheduled from virtual SIR resource'
...
* TN - output parameters added for NMIR project --------------------------------
  ...
  o_LirSent_TP(dt,ild)                        'SIR export from an island for each trade period'
  o_LirReceived_TP(dt,ild)                    'SIR received at an island for each trade period'
  o_LirEffReport_TP(dt,ild)                   'Effective FIR share for reporting to an island for each trade period'
  o_LirEffective_TP(dt,ild,riskC)             'Effective FIR share to an island for each trade period'
...
* Sets
$load i_offer i_trader i_bid i_node i_bus i_branch
$load i_branchConstraint i_MnodeConstraint
$load i_dateTimeTradePeriodMap i_tradePeriodOfferTrader i_tradePeriodOfferNode
$load i_tradePeriodBidTrader i_tradePeriodBidNode  i_tradePeriodNode
$load i_tradePeriodBusIsland i_tradePeriodBus i_tradePeriodNodeBus
$load i_tradePeriodBranchDefn i_tradePeriodRiskGenerator

* Parameters
$load i_day i_month i_year i_tradingPeriodLength
$load i_CVPvalues i_tradePeriodOfferParameter i_tradePeriodEnergyOffer
$load i_tradePeriodPLSRoffer i_tradePeriodPLSRfactor
$load i_tradePeriodTWDRoffer i_tradePeriodTWDRfactor
$load i_tradePeriodILRoffer i_tradePeriodILRfactor
$load i_tradePeriodNodeDemand i_tradePeriodEnergyBid
$load i_tradePeriodHVDCnode i_tradePeriodReferenceNode i_tradePeriodHVDCBranch
$load i_tradePeriodBranchParameter i_tradePeriodBranchCapacity
$load i_tradePeriodBranchOpenStatus
...
reserveRoundPower(tp,'LIR')         = reserveRoundPower(tp,'SIR')       ;
reserveShareEnabled(tp,'LIR')       = reserveShareEnabled(tp,'SIR')     ;
effectiveFactor(tp,ild,'LIR',riskC) = effectiveFactor(tp,ild,'SIR',riskC);
RMTreserveLimitTo(tp,ild,'LIR')     = RMTreserveLimitTo(tp,ild,'SIR')   ;
...
* TN - new reserve model testing
deficitReservePenalty(resC) $ (ord(resC) = 3)     = sum(i_CVP$(ord(i_CVP) = 4), i_CVPvalues(i_CVP)) ;
deficitReservePenalty_CE(resC) $ (ord(resC) = 3)  = sum(i_CVP$(ord(i_CVP) = 4), i_CVPvalues(i_CVP)) ;
deficitReservePenalty_ECE(resC)$ (ord(resC) = 3)  = sum(i_CVP$(ord(i_CVP) = 20), i_CVPvalues(i_CVP)) ;
* TN - new reserve model testing end
...
* TN - New reserve model testing
reserveOfferProportion(offer,trdBlk)
    = sum[ PLSofrCmpnt $ ( ord(PLSofrCmpnt) = 1 )
         , i_tradePeriodPLSRoffer(offer,trdBlk,PLSofrCmpnt) / 100 ] ;

reserveOfferMaximum(offer(tp,o),trdBlk,PLSRReserveType)
    = sum[ PLSofrCmpnt $ ( ord(PLSofrCmpnt) = 2 )
         , i_tradePeriodPLSRoffer(offer,trdBlk,PLSofrCmpnt) ];

reserveOfferMaximum(offer(tp,o),trdBlk,TWDRReserveType)
    = sum[ TWDofrCmpnt $ ( ord(TWDofrCmpnt) = 1 )
         , i_tradePeriodTWDRoffer(offer,trdBlk,TWDofrCmpnt) ];

reserveOfferMaximum(offer,trdBlk,ILReserveType)
    = sum[ ILofrCmpnt $ ( ord(ILofrCmpnt) = 1 )
         , i_tradePeriodILRoffer(offer,trdBlk,ILofrCmpnt) ];

reserveOfferPrice(offer,trdBlk,PLSRReserveType)
    = sum[ PLSofrCmpnt $ ( ord(PLSofrCmpnt) = 3 )
         , i_tradePeriodPLSRoffer(offer,trdBlk,PLSofrCmpnt) ];

reserveOfferPrice(offer,trdBlk,TWDRReserveType)
    = sum[ TWDofrCmpnt $ ( ord(TWDofrCmpnt) = 2 )
         , i_tradePeriodTWDRoffer(offer,trdBlk,TWDofrCmpnt) ];

reserveOfferPrice(offer,trdBlk,ILReserveType)
    = sum[ ILofrCmpnt $ ( ord(ILofrCmpnt) = 2 )
         , i_tradePeriodILRoffer(offer,trdBlk,ILofrCmpnt) ];

ReserveOfferFactor(offer,resC,PLSRReserveType)
    = i_tradePeriodPLSRfactor(offer,resC) ;

ReserveOfferFactor(offer,resC,TWDRReserveType)
    = i_tradePeriodTWDRfactor(offer,resC) ;

ReserveOfferFactor(offer,resC,ILReserveType)
    = i_tradePeriodILRfactor(offer,resC) ;

* Testing
*ReserveOfferFactor(tp,'WGN0331','FIR',ILReserveType) = 0.9 ;
*ReserveOfferFactor(offer,resC,ILReserveType) $ (ord(resC) = 3) = 0.9 ;

* TN - New reserve model testing end


* Only reserve offer block with a positive block limit is valid
validReserveOfferBlock(offer,trdBlk,resT)
    $ (reserveOfferMaximum(offer,trdBlk,resT) > 0) = yes ;
...
* TN - reserve model test
IslandRiskAdjustmentFactor(tp,ild,resC,riskC) $ (ord(resC) = 3)
    = sum[ i_reserveClass $ (ord(i_reserveClass) = 2)
                   , IslandRiskAdjustmentFactor(tp,ild,i_reserveClass,riskC) ] ;

FreeReserve(tp,ild,i_reserveClass,riskC) $ (ord(i_reserveClass) = 3)
   = sum[ resC $ (ord(resC) = 2), FreeReserve(tp,ild,resC,riskC) ] ;

HVDCpoleRampUp(tp,ild,i_reserveClass,riskC) $ (ord(i_reserveClass) = 3)
    = sum[ resC $ (ord(resC) = 2), HVDCpoleRampUp(tp,ild,resC,riskC) ] ;

IslandMinimumRisk(tp,ild,i_reserveClass,riskC) $ (ord(i_reserveClass) = 3)
    = sum[ resC $ (ord(resC) = 2),IslandMinimumRisk(tp,ild,resC,riskC) ] ;
* TN - reserve model test end
...
MnodeConstraint(tp,MnodeCstr)
    $ { sum[ (offer(tp,o),resT,resC)
      ...
      or
        sum[ (bid(tp,bd),resC)
           $ i_tradePeriodMnodeEnergyBidConstraintFactors(tp,MnodeCstr,bd), 1
           ]
      } = yes ;
...
MnodeReserveOfferConstraintFactors(MnodeConstraint,o,resT)
    = sum[ resC $ (ord(resC) = 2),
...
*   Ensure that all the invalid reserve blocks are set to zero for offers and purchasers
    RESERVEBLOCK.fx(offer(currTP,o),trdBlk,resT)
        $ (not validReserveOfferBlock(offer,trdBlk,resT)) = 0 ;

*   Reserve block maximum for offers and purchasers - Constraint 3.4.3.2.
    RESERVEBLOCK.up(validReserveOfferBlock(currTP,o,trdBlk,resT))
...
*   are either not connected to the grid or have no reserve quantity offered.
    RESERVE.fx(currTP,o,resT)
        $ (not sum[ trdBlk $ validReserveOfferBlock(currTP,o,trdBlk,resT), 1 ] ) = 0 ;
...
*       Offer output
        ...
        o_offerReserve_TP(dt,o) $ offer(currTP,o)
            = sum[ resT, RESERVE.l(currTP,o,resT) ] ;

        o_offerFIR_TP(dt,o) $ offer(currTP,o)
            = sum[ resC $ (ord(resC) = 1 ), RESERVECLASS.l(currTP,o,resC) ] ;

        o_offerSIR_TP(dt,o) $ offer(currTP,o)
            = sum[ resC $ (ord(resC) = 2 ), RESERVECLASS.l(currTP,o,resC) ] ;

        o_offerLIR_TP(dt,o) $ offer(currTP,o)
            = sum[ resC $ (ord(resC) = 3 ), RESERVECLASS.l(currTP,o,resC) ] ;
...
*       Offer output
        ...
        o_offerReserveBlock_TP(dt,o,trdBlk,resT)
            = RESERVEBLOCK.l(currTP,o,trdBlk,resT);
...
*       Violation reporting based on the CE and ECE
        ...
        o_LIRviolation_TP(dt,ild)
            = sum[ resC $ (ord(resC) = 3), o_ResViolation_TP(dt,ild,resC) ] ;
...
*       Island output
        ...
        o_LIRprice_TP(dt,ild) = sum[ resC $ (ord(resC) = 3)
                                , IslandReserveCalculation.m(currTP,ild,resC) ];

        o_ResPrice_TP(dt,ild) = o_FIRprice_TP(dt,ild)
                              + o_SIRprice_TP(dt,ild)
                              + o_LIRprice_TP(dt,ild) ;
        ...
        o_FirCleared_TP(dt,ild) = Sum[ resC $ (ord(resC) = 1)
                                            , ISLANDRESERVE.l(currTP,ild,resC)];

        o_SirCleared_TP(dt,ild) = Sum[ resC $ (ord(resC) = 2)
                                            , ISLANDRESERVE.l(currTP,ild,resC)];

        o_LIRcleared_TP(dt,ild) = Sum[ resC $ (ord(resC) = 3)
                                            , ISLANDRESERVE.l(currTP,ild,resC)];

        o_ResCleared_TP(dt,ild) = Sum[ o $ { offer(currTP,o)
                                         and offerIsland(currTP,o,ild) }
                                         , o_offerReserve_TP(dt,o) ] ;
...
* TN - The code below is added for NMIR project ================================
        ...
        If Sum[ resC $ (ord(resC) = 3), reserveShareEnabled(currTP,resC)] then

            o_LirSent_TP(dt,ild)
                = Sum[ (rd,resC) $ (ord(resC) = 3),
                       RESERVESHARESENT.l(currTP,ild,resC,rd) ];

            o_LirReceived_TP(dt,ild)
                = Sum[ (fd,resC) $ (ord(resC) = 3),
                       RESERVESHARERECEIVED.l(currTP,ild,resC,fd) ];

            o_LirEffective_TP(dt,ild,riskC)
                = Sum[ resC $ (ord(resC) = 3),
                       RESERVESHAREEFFECTIVE.l(currTP,ild,resC,riskC) ];

            o_LirEffReport_TP(dt,ild)
                = Smax[ (resC,riskC) $ (ord(resC)=3)
                     , RESERVESHAREEFFECTIVE.l(currTP,ild,resC,riskC) ];
        Endif;
...
*       conditions when reserve price = 0 - See below
        ...
        o_LIRreqd_TP(dt,ild) = sum[ resC $ (ord(resC)=3), o_ReserveReqd_TP(dt,ild,resC) ] ;
...
*       Virtual reserve
        ...
        o_LIRvrMW_TP(dt,ild) = sum[ resC $ (ord(resC) = 3)
                                  , o_vrResMW_TP(dt,ild,resC) ] ;
...
*           Scale the bus prices and reserve prices in the scarcity area
            ...
            scaledReservePrice(tp,ild) $ scarcityAreaIslandMap(sarea,ild)
                = scarcityScalingFactor(tp,sarea) * o_ResPrice_TP(dt,ild) ;

            scaledSIRprice(tp,ild) $ scarcityAreaIslandMap(sarea,ild)
                = scarcityScalingFactor(tp,sarea) * o_ResPrice_TP(dt,ild) ;

*           Allocate the scaled bus energy, reserve prices
            o_busPrice_TP(dt,b) $ sum[ busIsland(bus(tp,b),ild)
                                     $ scarcityAreaIslandMap(sarea,ild), 1
                                     ] = scaledbusPrice(tp,b) ;

            o_ResPrice_TP(dt,ild) $ scarcityAreaIslandMap(sarea,ild)
                                  = scaledReservePrice(tp,ild) ;
...
*   Island output
    ...
    o_islandReserveRevenue_TP(dt,ild) = sum[ resC, o_ResCleared_TP(dt,ild)
                                                 * o_ResPrice_TP(dt,ild)
                                                 * i_tradingPeriodLength/60 ];
